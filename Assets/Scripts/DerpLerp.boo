...
public class DerpLerp (MonoBehaviour):
    ...
    public static def MoveLocal(target as Transform, targetValue as Vector2, duration as single):
        lerper as DerpLerp = target.gameObject.AddComponent(DerpLerp)
        if targetValue != target.localPosition:
            lerper.StartCoroutine(lerper.MoveLocalRoutine(targetValue, duration))
    ...
    private def MoveLocalRoutine(targetValue as Vector2, duration as single) as IEnumerator:
        elapsedTime = 0f
        t = 0f

        startValue as Vector3 = transform.localPosition
        while elapsedTime < duration:
            yield WaitForSeconds(0.02f)
            elapsedTime += 0.02f
            t = elapsedTime/duration
            t = Mathf.Sin(t * Mathf.PI * 0.5f)
            transform.localPosition = Vector2.Lerp(startValue, targetValue * 1.1f , t - 0.1f)
        transform.localPosition = targetValue
        Destroy(self)
