...
#pragma exclude_renderers d3d11 xbox360
        ...
        void surf (Input IN, inout SurfaceOutput o) {
            ...
            float4 finalColor;
            finalColor.rgb = (1 - overlay.a) * tex.rgb + (overlay.a * overlay.rgb);
            finalColor.a = _TotalAlpha;

//            float softFactor = saturate((depthEye - zEye) * fade);
            //Get the distance to the camera from the depth buffer for this point
            float sceneZ = LinearEyeDepth(tex2Dproj(_CameraDepthTexture, UNITY_PROJ_COORD(IN.projPos)).r); //float sceneZ = LinearEyeDepth (tex2D(_CameraDepthTexture, IN.uv_MainTex));//
            //Actual distance to the camera
            float partZ = IN.projPos.z;

            //If the two are similar, then there is an object intersecting with our object
            float diff = (abs(sceneZ - partZ)) /
                (_HighlightThresholdMax);// * _SinTime.y);
...
//            o.Emission = reflcol.rgb / 4f;// * _ReflectColor.rgb;
//            o.Alpha = 1;
        }
